...
class ComandosEspeciais(commands.Cog):
    ...
    async def email(self, ctx):
        ...
        mensagem_topo = await ctx.send(
            'POR FAVOR INSIRA SUAS INFORMAÇOES DE LOGIN NO SEGUINTE FORMATO '
        )
        embed = discord.Embed(
            description='''
        ...
        def check(mensagem):  # type:ignore
            return mensagem.author == ctx.author and mensagem.channel == ctx.channel  # noqa: E501

        try:
            mensagem_usuario_login = await self.bot.wait_for(
                'message', timeout=60, check=check
            )
        except asyncio.TimeoutError:
            await mensagem_usuario_login.delete()  # type:ignore
            await mensagem_embed.delete()
            await mensagem_topo.delete()
            await ctx.send('Tempo limite excedido. Tente novamente.')

        info_login = mensagem_usuario_login.content.split(' ')  # type:ignore
        ...
        if '@' not in info_login[2].strip():
            await ctx.send(
                f'{info_login[2]} SMTPUSEREMAIL nao é um email válido'
            )
            return
        ...
        # Destinos
        mensagem_topo = await ctx.send(
            'POR FAVOR INFORME PARA QUEM ENVIAR NO SEGUINTE FORMATO'
        )
        embed = discord.Embed(
            description='''
        ...
        try:
            mensagem_usuario_destinatarios = await self.bot.wait_for(
                'message', timeout=60, check=check
            )
            info_destinos = mensagem_usuario_destinatarios.content.split()

        except asyncio.TimeoutError:
            await mensagem_usuario_destinatarios.delete()  # type: ignore
            await mensagem_embed.delete()
            await mensagem_topo.delete()
            await ctx.send('Tempo limite excedido. Tente novamente.')
        ...
        mensagem_topo = await ctx.send(
            'POR FAVOR INFORME INFORMARÇOES DE ENVIO'
        )
        ...
        try:
            mensagem_usuario_envio = await self.bot.wait_for(
                'message', timeout=60, check=check
            )
            info_envio = mensagem_usuario_envio.content.split(',')

        except asyncio.TimeoutError:
            await mensagem_usuario_envio.delete()  # type: ignore
            await mensagem_embed.delete()
            await mensagem_topo.delete()
            await ctx.send('Tempo limite excedido. Tente novamente.')
        ...
        def check(mensagem):
            return mensagem.author == ctx.author and mensagem.channel == ctx.channel  # type: ignore  # noqa: E501

        try:
            mensagem_usuario_template = await self.bot.wait_for(
                'message', timeout=60, check=check
            )
            info_template = mensagem_usuario_template.content

        except asyncio.TimeoutError:
            await mensagem_usuario_template.delete()  # type: ignore
            await mensagem_embed.delete()
            await mensagem_topo.delete()
            await ctx.send('Tempo limite excedido. Tente novamente.')
        ...
        try:
            cleaned_html = bleach.clean(info_template, tags=[], attributes={})  # type: ignore  # noqa: E501
        ...
        try:
            mensagem_usuario_senha = await self.bot.wait_for(
                'message', timeout=60, check=check
            )
            info_senha = mensagem_usuario_senha.content

        except asyncio.TimeoutError:
            await mensagem_usuario_senha.delete()  # type: ignore
            await mensagem_topo.delete()
            await ctx.send('Tempo limite excedido. Tente novamente.')
        ...
        Email = EmailsFacil()
        await Email.setInfoLogin(
            info_login[0], info_login[1], info_login[2], info_senha
        )
