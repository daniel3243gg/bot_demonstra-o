...
"""
ABAIXO ESTÁ O CÓDIGO QUE ATUALIZA SUAS CONFIGS COM O BANCO DE DADOS CORRETO.
"""
...
try:
    with open('config.json', 'r+') as local_file:
        ...
        if not local_data:
            # Inicializa como um dicionário vazio se o arquivo estiver vazio.
            local_data = {}
...
if response.status_code == 200:
    try:
        # Converte o conteúdo da resposta para um objeto Python
        # (por exemplo, um dicionário)
        api_data = json.loads(response.text)

        # Criar uma cópia dos dados locais para evitar a substituição total
        updated_data = local_data.copy() if local_data else {}  # type: ignore
    ...
    print(
        f'A requisição falhou com o código de status: {response.status_code}'
    )
...
async def setup():
    ...
    client.config = config  # type: ignore
...
if __name__ == "__main__":
    ...
    try:
        client.run(client.config['token_bot'])  # type: ignore
    except KeyError:
        print('INSIRA NO config.json O SEU TOKEN DE BOT COM a chave "token_bot": "seu token"')  # noqa: E501
